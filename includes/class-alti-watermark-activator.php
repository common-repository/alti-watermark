...
class Alti_Watermark_Activator extends Alti_Watermark {
	...
	*/
	public function generate_empty_watermark() {
		if( !is_admin() ) exit;
		if( self::check_watermark_folder() !== false && !file_exists( WP_PLUGIN_DIR . '/' . 'alti-watermark' . '-data' . '/watermark.png' )) {
			...
			imagepng( $empty_watermark, WP_PLUGIN_DIR . '/' . 'alti-watermark' . '-data' . '/watermark.png' );
	...
	public function reactivate_previous_htaccess() {
		if( !is_admin() ) exit;
		if( file_exists( self::get_uploads_dir() . '/' . 'alti-watermark' . '.old.htaccess' ) && !file_exists( self::get_uploads_dir() . '/' . '.htaccess' )) {
			rename(self::get_uploads_dir() . '/' . 'alti-watermark' . '.old.htaccess', self::get_uploads_dir() . '/' . '.htaccess');
		}
